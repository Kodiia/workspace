<script>
  ...
  import { width, primaryColor } from '$lib/store';

    let container 
    function createSVG(width = 100, height = 100, cutHeight = 20, strokeColor = `hsl(${primaryColor})`, fillColor = `hsl(${primaryColor} + , 20%)`, contourState = false, containerElement = '', id = 'card', SVGscale = 1){
...
<div bind:this = {container} bind:clientWidth={containerWidth} style='width: 100%'>
    <svg width='{size}' height='{size}' viewBox='0 0 {containerWidth} {containerWidth}'>
        <g>
            <path d={SVGpath} fill='hsl({$primaryColor + ", 30%"})' stroke='hsl({$primaryColor + ", 50%"})'></path>
        </g>
    </svg>
